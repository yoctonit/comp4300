...
void Game::spawnPlayer()
{
    ...
    e->cTransform = std::make_shared<CTransform>(Vec2(200.0f, 200.0f), Vec2(1.0f, 1.0f), 0.0f);

    // The entity's shape will have radius 32, 8 sides, dark grey fill, and red outline of thickness 4
    e->cShape = std::make_shared<CShape>(32.0f, 8, sf::Color(10,10,10), sf::Color(255,0,0), 4.0f);

    // Add an input component to the player so that we can use inputs
    entity->cInput = std::make_shared<Input>();
...
void Game::spawnEnemy()
{
    ...
    m_lastEnenySpawntime = m_currentFrame;
}
...
void Game::sUserInput()
{
    ...
    while (m_window.pollEvent(event))
    {
        ...
        ImGui::SMFL::ProcessEvent(m_window, event);

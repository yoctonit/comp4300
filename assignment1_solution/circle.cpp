...
Circle::Circle(const CircleConfig &sc, const FontConfig &fc, sf::Font &font, WindowConfig wc) : Shape() {
    m_type = "circle";
    m_name = sc.name;
...
//    m_text.setPosition(m_posX + m_sizeX, m_posY + m_sizeY);

    m_shape.setPosition(m_posX, m_posY);
    m_shape.setRadius(m_sizeX);
    m_shape.setFillColor(sf::Color{m_R, m_G, m_B});
}

void Circle::CalculateNamePosition() {
    sf::FloatRect textRect = m_text.getLocalBounds();
    m_text.setOrigin(textRect.left + textRect.width / 2.0f, textRect.top + textRect.height / 2.0f);
    m_text.setPosition(m_posX + m_sizeX * m_scale, m_posY + m_sizeY * m_scale);
}
...
void Circle::Update() {
    ...
    if (m_posX + 2.0f * m_sizeX * m_scale > m_windowWidth) {
        m_speedX *= -1.0;
    }
    if (m_posY < 0) {
        m_speedY *= -1.0;
    }
    if (m_posY + 2.0f * m_sizeY * m_scale > m_windowHeight) {
        m_speedY *= -1.0;
    }
